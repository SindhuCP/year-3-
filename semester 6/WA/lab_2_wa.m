...
mu=input('Enter relative permittivity value (mu_r): ');
epsilon=input('Enter relative permeability value (eplilon_r):');
f=input('Enter frequency value in GHz: ');
f=f*(10^9);
mu0=4*pi*(10^(-7));
e0=8.854*10^(-12);
fc=(1/(2*pi*sqrt(mu*epsilon*mu0*e0)))*sqrt(((m*pi/a)^2)+((n*pi/b)^2));
k=2*pi*f*sqrt(mu*epsilon*mu0*e0);
kc=sqrt(((m*pi/a)^2)+((n*pi/b)^2));
beta=sqrt((k^2)-(kc^2));
lambdag=2*pi/beta
eta=sqrt((mu*mu0)/(epsilon*e0));
vg=(3*10^8)/((sqrt(mu*epsilon))*(sqrt(1-(fc/f)^2)))
if choice == 1
    zg=eta/(sqrt(1-(fc/f)^2))
elseif choice == 2
    zg=eta*sqrt(1-((fc/f)^2))
else
